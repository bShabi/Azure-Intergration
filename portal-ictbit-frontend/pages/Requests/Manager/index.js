...
export default function ManagerRequestPage({ Data, DataSetting, budgetData }) {
  ...
  useEffect(() => {
    ...
    console.log('jwt2', cookies.token);
  ...
  function pushUpdateNotification() {
    return socket.emit('updateApiCall');
  }
...
export async function getStaticProps(ctx) {
  // var test = cookieCutter.get('token');
  ...
  const testRes = {};
  /**/
  /**Change name function and add header jwt/ */
  var Data = await requestManagerApi.getAllBudgets();
